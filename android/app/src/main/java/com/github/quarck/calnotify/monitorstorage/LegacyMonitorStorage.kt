...
/**
 * Legacy SQLiteOpenHelper-based MonitorStorage.
...
class LegacyMonitorStorage(val context: Context)
    ...
    override fun addAlert(entry: MonitorEventAlertEntry)
            = synchronized(LegacyMonitorStorage::class.java) { writableDatabase.customUse { impl.addAlert(it, entry) } }

    override fun addAlerts(entries: Collection<MonitorEventAlertEntry>)
            = synchronized(LegacyMonitorStorage::class.java) { writableDatabase.customUse { impl.addAlerts(it, entries) } }
    ...
    override fun deleteAlerts(entries: Collection<MonitorEventAlertEntry>)
            = synchronized(LegacyMonitorStorage::class.java) { writableDatabase.customUse { impl.deleteAlerts(it, entries) } }

    override fun deleteAlert(eventId: Long, alertTime: Long, instanceStart: Long)
            = synchronized(LegacyMonitorStorage::class.java) { writableDatabase.customUse { impl.deleteAlert(it, eventId, alertTime, instanceStart) } }

    override fun deleteAlertsMatching(filter: (MonitorEventAlertEntry) -> Boolean)
            = synchronized(LegacyMonitorStorage::class.java) { writableDatabase.customUse { impl.deleteAlertsMatching(it, filter) } }

    override fun updateAlert(entry: MonitorEventAlertEntry)
            = synchronized(LegacyMonitorStorage::class.java) { writableDatabase.customUse { impl.updateAlert(it, entry) } }

    override fun updateAlerts(entries: Collection<MonitorEventAlertEntry>)
            = synchronized(LegacyMonitorStorage::class.java) { writableDatabase.customUse { impl.updateAlerts(it, entries) } }

    override fun getAlert(eventId: Long, alertTime: Long, instanceStart: Long): MonitorEventAlertEntry?
            = synchronized(LegacyMonitorStorage::class.java) { readableDatabase.customUse { impl.getAlert(it, eventId, alertTime, instanceStart) } }

    override fun getInstanceAlerts(eventId: Long, instanceStart: Long): List<MonitorEventAlertEntry>
            = synchronized(LegacyMonitorStorage::class.java) { readableDatabase.customUse { impl.getInstanceAlerts(it, eventId, instanceStart) } }

    override fun getNextAlert(since: Long): Long?
            = synchronized(LegacyMonitorStorage::class.java) { readableDatabase.customUse { impl.getNextAlert(it, since) } }

    override fun getAlertsAt(time: Long): List<MonitorEventAlertEntry>
            = synchronized(LegacyMonitorStorage::class.java) { readableDatabase.customUse { impl.getAlertsAt(it, time) } }

    override val alerts: List<MonitorEventAlertEntry>
        get() = synchronized(LegacyMonitorStorage::class.java) { readableDatabase.customUse { impl.getAlerts(it) } }

    override fun getAlertsForInstanceStartRange(scanFrom: Long, scanTo: Long): List<MonitorEventAlertEntry>
            = synchronized(LegacyMonitorStorage::class.java) { readableDatabase.customUse { impl.getAlertsForInstanceStartRange(it, scanFrom, scanTo) } }
