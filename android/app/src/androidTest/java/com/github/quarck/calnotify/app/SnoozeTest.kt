...
/**
 * Instrumented tests for snooze functionality in ApplicationController
 */
@RunWith(AndroidJUnit4::class)
class SnoozeTest {
    private val LOG_TAG = "SnoozeTest"
    ...
    @Before
    fun setup() {
        DevLog.info(LOG_TAG, "Setting up SnoozeTest")
    ...
    @After
    fun cleanup() {
        DevLog.info(LOG_TAG, "Cleaning up SnoozeTest")
        unmockkAll()
        cleanupStorage()
    }
    ...
    private fun cleanupStorage() {
        try {
            ...
            DevLog.error(LOG_TAG, "Cleanup error: ${e.message}")
        }
    }
    ...
    fun testSnoozeEventBasic() {
        DevLog.info(LOG_TAG, "Running testSnoozeEventBasic")
    ...
    fun testSnoozeEventNonExistent() {
        DevLog.info(LOG_TAG, "Running testSnoozeEventNonExistent")
    ...
    fun testSnoozeEventPastTimeFallback() {
        DevLog.info(LOG_TAG, "Running testSnoozeEventPastTimeFallback")
        ...
        // Snooze with a negative delay that would result in past time
        val snoozeDelay = -2 * 60 * 60 * 1000L // -2 hours
    ...
    fun testSnoozeAllEventsBasic() {
        DevLog.info(LOG_TAG, "Running testSnoozeAllEventsBasic")
    ...
    fun testSnoozeAllEventsWithSearchQuery() {
        DevLog.info(LOG_TAG, "Running testSnoozeAllEventsWithSearchQuery")
    ...
    fun testSnoozeAllEventsEmpty() {
        DevLog.info(LOG_TAG, "Running testSnoozeAllEventsEmpty")
    ...
    fun testSnoozeAllCollapsedEventsOnly() {
        DevLog.info(LOG_TAG, "Running testSnoozeAllCollapsedEventsOnly")
    ...
    fun testSnoozeOnlyVisibleEvents() {
        DevLog.info(LOG_TAG, "Running testSnoozeOnlyVisibleEvents")

        // Given - one already snoozed, one not snoozed
        val alreadySnoozed = createTestEvent(
            eventId = 500L,
            snoozedUntil = baseTime + 60 * 60 * 1000L
        )
